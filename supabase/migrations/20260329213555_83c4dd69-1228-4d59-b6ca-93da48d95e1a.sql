-- Migrate existing "lead" accounts to "pending_info"
UPDATE accounts SET status = 'pending_info' WHERE status = 'lead';

-- Update the auto_update_account_status trigger to remove "lead" references
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'quotes' THEN
    ...
    IF NEW.status = 'submitted' AND (OLD IS NULL OR OLD.status != 'submitted') THEN
      IF v_current_status IN ('pending_info', 'info_complete') THEN
